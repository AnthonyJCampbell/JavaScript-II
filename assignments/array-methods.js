...
// Problem 1
// The organizers want to know who the biggest donors were so they can be thanked accordingly. Management has asked you to prepare a list of all runners who donated more than $200
let sortedDonors = [];
runners.filter((runner) => {
    if (runner.donation >= 200) {
        sortedDonors.push(`${runner.first_name} ${runner.last_name}`);
    }
})
...
// Problem 2
// We want to send every runner a quick thank you-email for their pariciption. Make sure each runner gets his or her personalized message logged to the console!
let outgoingMail = []
runners.forEach((runner) => {
    outgoingMail.push({
        'id': runner.id,
        'to': runner.email,
        'message': `Thank you, ${runner.first_name} ${runner.last_name} for your participation and your $${runner.donation} donation!`
    })
})
console.log(outgoingMail);
